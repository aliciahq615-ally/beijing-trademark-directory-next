    ...
    return data.companies
      ...
      .filter(({ company }) => {
        return [
          ...
          company.entries.map((entry) => `${entry.mark} ${entry.registration}`).join(" "),
        ]
          .join(" ")
          .toLowerCase()
          .includes(normalized);
          ...
          <div className="company-list">
            {filteredCompanies.slice(0, 120).map(({ company, index }) => (
              <button
                ...
                key={`${company.book}-${company.name}`}
                onClick={() => setSelectedIndex(index)}
                type="button"
              >
                <strong>{company.name}</strong>
                <span>
                  {company.bookName} · {company.records} 条记录
                </span>
              </button>
            ))}
          </div>
            ...
            <form className="editor-form">
              ...
              <label>
                ...
                <textarea
                  ...
                  spellCheck={false}
                />
                <small>每行一条：商标名 | 注册号 | 类别号 | 类别名称</small>
              </label>
              <label>
                首页滚动 Logo
                <textarea
                  ...
                  spellCheck={false}
                />
                <small>每行一个：显示名称 | 域名，例如 小米 | mi.com</small>
              </label>
            </form>
          ) : null}
        </section>
...
function normalizeCatalog(data: CatalogData): CatalogData {
  ...
  const companies = data.companies.map((company) => {
    ...
    return {
      ...company,
      bookName: bookNames[company.book],
...
function entriesToText(entries: TrademarkEntry[]) {
  return entries.map((entry) => `${entry.mark || ""} | ${entry.registration || ""} | ${entry.class || ""} | ${entry.className || ""}`).join("\n");
}
...
function textToEntries(text: string): TrademarkEntry[] {
  return text
    ...
    .filter(Boolean)
    .map((line) => {
      const [mark = "", registration = "", classId = "", className = ""] = line.split("|").map((item) => item.trim());
      return { mark, registration, class: classId, className };
    });
}

function logosToText(logos: LogoItem[]) {
  return logos.map((logo) => `${logo.name} | ${logo.domain}`).join("\n");
}
...
function textToLogos(text: string): LogoItem[] {
  return text
    ...
    .filter(Boolean)
    .map((line) => {
      const [name = "", domain = ""] = line.split("|").map((item) => item.trim());
      return { name, domain };
    });
}
